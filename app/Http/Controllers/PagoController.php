    ...
    public function index(Request $request)
{
    ...
    try {
        ...
        if ($request->filled('codigo')) {
            $query->where('codigo_pago', 'like', '%' . $request->codigo . '%');
        }

        // Búsqueda por estudiante
        if ($request->filled('estudiante')) {
            $query->whereHas('inscripcion.estudiante', function($q) use ($request) {
                $q->where('nombres', 'like', '%' . $request->estudiante . '%')
                  ->orWhere('apellidos', 'like', '%' . $request->estudiante . '%')
                  ->orWhere('dni', 'like', '%' . $request->estudiante . '%');
            });
        }
