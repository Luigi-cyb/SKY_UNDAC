...
{
    $request->validate([
        'archivo' => 'required|mimes:xlsx,xls|max:5120',
    ]);
    ...
    try {
        ...
        if ($failures->count() > 0) {
            ...
            \Log::error('Errores en importación', ['errores' => $errores]);
            
            return redirect()->back()
                ->with('error', '⚠️ Algunos estudiantes no se importaron: ' . implode(' | ', $errores));
        }
...
public function descargarPlantilla()
{
    ...
    foreach ($headers as $index => $header) {
        $sheet->setCellValue(chr(65 + $index) . '1', strtoupper($header));
    }
