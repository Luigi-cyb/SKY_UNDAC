    ...
    public function index(Request $request)
{
    ...
    try {
        ...
        $cursos = Curso::whereIn('estado', ['en_curso', 'convocatoria'])
            ->orderBy('nombre')
            ->get();

        // 🔍 Query de asistencias con filtros
        $query = Asistencia::with(['inscripcion.estudiante', 'curso']);
        ...
        $asistencias = $query->orderBy('fecha_sesion', 'desc')
            ->orderBy('numero_sesion', 'desc')
            ->paginate(15)
            ->withQueryString();

        // 📈 Estadísticas generales
        $totalCursos = Curso::whereIn('estado', ['en_curso', 'convocatoria'])->count();
        $totalRegistros = Asistencia::count();
