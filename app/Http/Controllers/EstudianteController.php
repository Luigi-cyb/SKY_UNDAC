    ...
    public function index(Request $request)
{
    ...
    // 🔍 UN SOLO FILTRO: busca en DNI, nombre, código, correo
    ...
    // 📊 Ordenamiento
    $orderBy = $request->get('ordenar', 'apellidos');
    $orderDir = $request->get('direccion', 'asc');
...
public function mostrarInscripcion(Curso $curso)
{
    ...
    $yaInscrito = $estudiante->inscripciones()
        ->where('curso_id', $curso->id)
        ->whereIn('estado', ['Confirmada', 'confirmada', 'Pendiente', 'pendiente', 'En Curso'])
        ->exists();
    ...
    // ✅ CORRECCIÓN: Contar SOLO inscripciones CONFIRMADAS o EN CURSO
    $inscritosCount = $curso->inscripciones()
        ->whereIn('estado', ['Confirmada', 'En Curso'])
        ->count();

    // ✅ CORRECCIÓN: Usar cupo_maximo (campo correcto de la BD)
    $cuposDisponibles = $curso->cupo_maximo - $inscritosCount;

    // Verificar que haya cupos
    if ($cuposDisponibles <= 0) {
        return redirect()->route('estudiantes.cursos-disponibles')
            ->with('error', 'Este curso ya no tiene cupos disponibles.');
    }
