    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-xl mb-6">
                ...
                <div class="p-6">
                    <form method="GET" action="{{ route('certificados.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
    <!-- Búsqueda General -->
    <div class="md:col-span-2">
        <label for="search" class="block text-sm font-bold text-gray-700 mb-2">🔍 Búsqueda General</label>
        <input type="text" name="search" id="search" 
               value="{{ request('search') }}"
               placeholder="DNI, nombre, código certificado, curso..."
...
@push('scripts')
<script>
    // ✅ Guardar logs en localStorage para verlos después
    const logs = [];
    ...
    function abrirModalSubir(certificadoId) {
        agregarLog('✅ Abriendo modal para certificado: ' + certificadoId);
        ...
        agregarLog('✅ Action establecido a: ' + form.action);
    ...
    function mostrarNombreArchivo(input) {
        ...
        if (input.files && input.files[0]) {
            ...
            agregarLog('✅ Archivo seleccionado: ' + input.files[0].name + ' (' + input.files[0].size + ' bytes)');
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (form) {
            form.addEventListener('submit', async function(e) {
                ...
                agregarLog('');
                agregarLog('═══════════════════════════════════');
                agregarLog('📝 FORMULARIO ENVIADO');
                agregarLog('═══════════════════════════════════');
                agregarLog('Action: ' + this.action);
                
                // Obtener archivo
                const fileInput = this.querySelector('input[name="pdf_firmado"]');
                if (!fileInput.files || !fileInput.files[0]) {
                    agregarLog('❌ NO HAY ARCHIVO SELECCIONADO');
                    alert('❌ Por favor selecciona un archivo PDF');
                    return;
                }
                
                const file = fileInput.files[0];
                agregarLog('📄 Archivo: ' + file.name);
                agregarLog('📄 Tamaño: ' + file.size + ' bytes');
                agregarLog('📄 Tipo: ' + file.type);
                
                // Validar tamaño
                if (file.size > 5 * 1024 * 1024) {
                    agregarLog('❌ ARCHIVO MÁS GRANDE QUE 5MB');
                ...
                agregarLog('📤 Enviando request...');
                agregarLog('📤 URL: ' + this.action);
                agregarLog('📤 Método: POST');
                ...
                try {
                    ...
                    agregarLog('📥 Response recibido');
                    agregarLog('📥 Status: ' + response.status);
                    agregarLog('📥 OK: ' + response.ok);
                    
                    if (response.ok) {
                        agregarLog('✅ SUCCESS - Recargando página en 1 segundo');
                        alert('✅ Certificado firmado subido correctamente');
                        
                        // Esperar 1 segundo antes de recargar
                        await new Promise(resolve => setTimeout(resolve, 1000));
                        location.reload();
                    } else if (response.status === 403) {
                        agregarLog('🔴 ERROR 403 FORBIDDEN');
                        alert('❌ No tienes permisos (Error 403)');
                    } else if (response.status === 404) {
                        agregarLog('🔴 ERROR 404 NOT FOUND');
                        const text = await response.text();
                        agregarLog('Respuesta: ' + text.substring(0, 200));
                        alert('❌ Ruta no encontrada (Error 404)');
                    } else if (response.status === 422) {
                        agregarLog('🔴 ERROR 422 VALIDATION');
                        const data = await response.json();
                        agregarLog('Errores: ' + JSON.stringify(data.errors));
                        alert('❌ Error de validación');
                    } else {
                        agregarLog('🔴 ERROR ' + response.status);
                        const text = await response.text();
                        agregarLog('Respuesta: ' + text.substring(0, 200));
                        alert('❌ Error del servidor: ' + response.status);
                    }
                } catch (error) {
                    agregarLog('🔴 CATCH ERROR: ' + error.message);
                    agregarLog('Stack: ' + error.stack);
    ...
    // ✅ Mostrar logs cuando se carga la página
    window.addEventListener('load', function() {
        const storedLogs = localStorage.getItem('uploadLogs');
        if (storedLogs) {
            const logs = JSON.parse(storedLogs);
            console.log('📋 LOGS DEL ÚLTIMO UPLOAD:');
            console.log('═══════════════════════════════════');
