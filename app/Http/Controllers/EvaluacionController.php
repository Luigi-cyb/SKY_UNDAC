...
class EvaluacionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $query = Evaluacion::with(['curso.modalidad', 'curso.categoria'])
            ...
            $allowedSorts = ['nombre', 'tipo', 'peso_porcentaje', 'fecha_evaluacion', 'created_at'];
            ...
            $stats = [
                ...
                'activas' => Evaluacion::where('activo', true)->count(),
                'inactivas' => Evaluacion::where('activo', false)->count(),
                'con_calificaciones' => Evaluacion::whereHas('calificaciones')->count(),
                'pendientes' => Evaluacion::where('activo', true)
                    ->doesntHave('calificaciones')
                    ->count(),
            ];
            ...
            return view('evaluaciones.index', compact('evaluaciones', 'stats', 'cursos'));

        } catch (\Exception $e) {
            Log::error('Error en EvaluacionController@index: ' . $e->getMessage());
            return redirect()->back()->with('error', 'Error al cargar las evaluaciones.');
        }
    }
   ...
   public function create(Request $request)
{
    ...
    // 📊 Si hay un curso seleccionado, mostrar peso disponible
    $pesoDisponible = 100;
    if ($curso_id) {
