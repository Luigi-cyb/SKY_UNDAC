...
public function generarMasivo(Request $request)
{
    ...
    try {
        ...
        foreach ($request->inscripciones as $inscripcionId) {
            ...
            // Calcular asistencia si no existe
...
$codigoQr = str_random(32); // Código QR único
$urlCertificado = route('certificado.publico', $codigoQr); // ← La URL pública

$certificado = Certificado::create([
    'inscripcion_id' => $inscripcion->id,
    'codigo_certificado' => $numeroCertificado,
    'codigo_qr' => $codigoQr,
    'pdf_url' => $urlCertificado, // ← Guardar la URL
    // ... resto de campos
]);

            // Actualizar inscripción con nota final
            $inscripcion->update([
                'nota_final' => $notaFinal,
        ...
        $mensaje = "✓ Se generaron {$contadorGenerados} certificados correctamente.";
        if (count($errores) > 0) {
    ...
    public function descargar(Certificado $certificado)
{
    ...
    // ✅ Verificar si es admin/comité/docente (pueden descargar cualquier certificado)
    ...
    // ⭐ Redirigir a la vista pública del certificado
...
{
    try {
        // Buscar certificado por código QR
